...
extern "C"
{
    ...
    JNIEXPORT void Java_com_speex_Speex_intSpeex(JNIEnv *env,jobject thiz,jint quality)
    {
        //判断是否开启，避免重复开启
        if(code_open)
        {
...
*/
    ...
    //编码
    JNIEXPORT jbyteArray Java_com_speex_Speex_encodeSpeex(JNIEnv *env,jobject thiz,jshortArray lin)
    {
        if(!enc_state)
        {
            return 0;
    ...
    JNIEXPORT jint Java_com_speex_Speex_getEncodeSize(JNIEnv *env,jobject thiz)
    {
        if(!code_open)
        {
            return 0;
    ...
    JNIEXPORT jint Java_com_speex_Speex_getDecodeSize(JNIEnv *env,jobject thiz)
    {
        if(!code_open)
        {
            return 0;
    ...
    JNIEXPORT void Java_com_speex_Speex_closeSpeex(JNIEnv *env,jobject thiz)
    {
        if(!code_open)
        {
            return;
